...
import { executeMyFatoorahPayment, getMyFatoorahPaymentStatus } from "./myclasses.utils";
...
export const initiateClassPayment = async (userId: string, payload: any) => {
    ...
    if (classType === "CLASS") {
        ...
        const alreadyEnrolled = await MyClassModel.findOne({
            student: userId,
            classId: classId,
            status: "PAID",
        });
    ...
    // Create record in MyClass
    const myClass = await MyClassModel.create({
        student: userId,
        teacher: teacherId,
        amount: finalAmount,
        currency,
...
export const verifyClassPayment = async (paymentId: string) => {
    ...
    const myClass = await MyClassModel.findOne({ invoiceId: invoiceData.InvoiceId });

    if (!myClass) throw new ApiError(httpStatus.NOT_FOUND, "Payment record not found");

    if (invoiceData.InvoiceStatus === "Paid") {
        myClass.status = "PAID";
        myClass.paymentId = paymentId;
        ...
        if (myClass.classType === "CLASS") {
            await ClassModel.findByIdAndUpdate(myClass.classId, {
...
export const getStudentClasses = async (studentId: string, query: any) => {
    ...
    const result = await MyClassModel.find(filters).populate("teacher", "name email avatar").populate("slotId").sort({ createdAt: -1 }).skip(skip).limit(Number(limit)).lean();
    ...
    const total = await MyClassModel.countDocuments(filters);
    return {
...
export const getTeacherClasses = async (teacherId: string, query: any) => {
    ...
    const result = await MyClassModel.find(filters).populate("student", "name email avatar").populate("slotId").sort({ createdAt: -1 }).skip(skip).limit(Number(limit)).lean();
