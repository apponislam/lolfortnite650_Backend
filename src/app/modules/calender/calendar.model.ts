...
// import { ITeacherAvailability, ISlot, IBooking } from "./calendar.interface";

// /* TeacherAvailability */
// const availabilitySlotSchema = new Schema({ startTime: String, endTime: String }, { _id: false });
// const dayAvailabilitySchema = new Schema({ day: { type: String, enum: ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"], required: true }, slots: [availabilitySlotSchema] }, { _id: false });
// const teacherAvailabilitySchema = new Schema({ teacher: { type: Schema.Types.ObjectId, ref: "User", required: true, unique: true, index: true }, availability: [dayAvailabilitySchema] }, { timestamps: true });
// export const TeacherAvailability = model<ITeacherAvailability>("TeacherAvailability", teacherAvailabilitySchema);

// /* Slot */
// const slotSchema = new Schema(
//     { teacher: { type: Schema.Types.ObjectId, ref: "User", required: true, index: true }, date: { type: Date, required: true, index: true }, startTime: { type: String, required: true }, endTime: { type: String, required: true }, isBooked: { type: Boolean, default: false, index: true }, booking: { type: Schema.Types.ObjectId, ref: "Booking", default: null } },
//     { timestamps: true },
// );
// slotSchema.index({ teacher: 1, date: 1, startTime: 1 }, { unique: true });
// slotSchema.index({ teacher: 1, date: 1, isBooked: 1 });
